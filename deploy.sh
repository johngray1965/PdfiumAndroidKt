...
if [ -z "$JRELEASER_GPG_PUBLIC_KEY" ]; then
  echo "❌ ERROR: JRELEASER_GPG_PUBLIC_KEY is not set."
  echo "-----------------------------------------------------"
  echo "Please set the required environment variables for signing."
  echo "You can use the following commands:"
  echo ""
  echo "  export JRELEASER_GPG_PUBLIC_KEY=\$(gpg --export --armor YOUR_KEY_ID)"
  echo "  export JRELEASER_GPG_SECRET_KEY=\$(gpg --export-secret-keys --armor YOUR_KEY_ID)"
  echo "  export JRELEASER_GPG_PASSPHRASE='your-secret-gpg-passphrase'"
  echo ""
...
if [ -z "$JRELEASER_GPG_SECRET_KEY" ]; then
  echo "❌ ERROR: JRELEASER_GPG_SECRET_KEY is not set."
...
if [ -z "$JRELEASER_GPG_PASSPHRASE" ]; then
  echo "❌ ERROR: JRELEASER_GPG_PASSPHRASE is not set."
  echo "--> Please check the instructions above and ensure all three variables are exported."
  exit 1
fi
